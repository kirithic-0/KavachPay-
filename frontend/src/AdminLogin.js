...
export default function AdminLogin({ onLogin, onBack }) {
    ...
    const handleSecurityVerify = () => {
        ...
        if (securityAnswer.toLowerCase().trim() !== SECURITY_ANSWER) {
            setSecurityError('Incorrect answer. Please try again.'); return;
        }
        setSecurityError('');
        setLoading(true);
    ...
    const labelStyle = {
        display: 'block', color: '#aaa', fontWeight: '600',
        fontSize: '13px', marginBottom: '7px', letterSpacing: '0.3px'
    };
        ...
        <div style={{ backgroundColor: '#0D1B2A', minHeight: '100vh', fontFamily: 'Arial', display: 'flex', flexDirection: 'column' }}>
            <div style={{ backgroundColor: '#1A1A2E', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <button onClick={onBack}
                    style={{ backgroundColor: 'transparent', color: '#aaa', border: '1px solid rgba(255,255,255,0.15)', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
                    Back to Home
                </button>
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
                <div style={{ width: '100%', maxWidth: '400px' }}>
                    ...
                    <div style={{ backgroundColor: 'rgba(240,165,0,0.08)', border: '1px solid rgba(240,165,0,0.2)', borderRadius: '10px', padding: '10px 16px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <p style={{ color: '#F0A500', fontSize: '12px', letterSpacing: '0.3px' }}>Restricted access — Authorized personnel only</p>
                    </div>
                        ...
                        <div style={{ marginBottom: '8px' }}>
                            ...
                            <div style={{ position: 'relative' }}>
                                <input type={showPassword ? 'text' : 'password'} placeholder="Enter admin password" value={password}
                                    ...
                                    onKeyDown={e => e.key === 'Enter' && handleLogin()}
                                    style={{ ...darkInput, paddingRight: '60px' }} />
                                <button onClick={() => setShowPassword(!showPassword)}
                                    style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#666', fontSize: '13px' }}>
                ...
                <div style={{ width: '100%', maxWidth: '400px' }}>
                    ...
                    <div style={{ backgroundColor: '#1A1A2E', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <input type="email" placeholder="admin@kavachpay.in" value={forgotEmail}
                                onChange={e => { setForgotEmail(e.target.value); setForgotError(''); }}
                                onKeyDown={e => e.key === 'Enter' && handleForgotSubmit()}
                                style={{ ...darkInput, border: `1.5px solid ${forgotError ? 'rgba(220,38,38,0.5)' : 'rgba(255,255,255,0.08)'}` }} />
                            {forgotError && <p style={{ color: '#F87171', fontSize: '12px', marginTop: '6px' }}>{forgotError}</p>}
                        </div>

                        <button onClick={handleForgotSubmit}
                            style={{ width: '100%', backgroundColor: '#F0A500', color: 'white', padding: '14px', borderRadius: '10px', border: 'none', fontSize: '15px', fontWeight: 'bold', cursor: 'pointer' }}>
                            Continue
                        </button>
                    </div>
                </div>
    ...
    if (screen === 'security') return (
        <div style={{ backgroundColor: '#0D1B2A', minHeight: '100vh', fontFamily: 'Arial', display: 'flex', flexDirection: 'column' }}>
            <div style={{ backgroundColor: '#1A1A2E', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <button onClick={() => { setScreen('forgot'); setSecurityAnswer(''); setSecurityError(''); }}
                    style={{ backgroundColor: 'transparent', color: '#aaa', border: '1px solid rgba(255,255,255,0.15)', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
                    Back
                </button>
            </div>
                    ...
                    <div style={{ backgroundColor: '#1A1A2E', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <input type="text" placeholder="Enter your answer" value={securityAnswer}
                                ...
                                style={{ ...darkInput, border: `1.5px solid ${securityError ? 'rgba(220,38,38,0.5)' : 'rgba(255,255,255,0.08)'}` }} />
                            {securityError && <p style={{ color: '#F87171', fontSize: '12px', marginTop: '6px' }}>{securityError}</p>}
                        </div>

                        <button onClick={handleSecurityVerify} disabled={loading}
                            style={{ width: '100%', backgroundColor: loading ? '#B87A00' : '#F0A500', color: 'white', padding: '14px', borderRadius: '10px', border: 'none', fontSize: '15px', fontWeight: 'bold', cursor: loading ? 'default' : 'pointer' }}>
    ...
    if (screen === 'newpassword') return (
        <div style={{ backgroundColor: '#0D1B2A', minHeight: '100vh', fontFamily: 'Arial', display: 'flex', flexDirection: 'column' }}>
            <div style={{ backgroundColor: '#1A1A2E', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <button onClick={() => { setScreen('security'); setResetError(''); }}
                    style={{ backgroundColor: 'transparent', color: '#aaa', border: '1px solid rgba(255,255,255,0.15)', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
                    Back
                </button>
            </div>
                    ...
                    <div style={{ backgroundColor: '#1A1A2E', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <div style={{ marginBottom: '24px' }}>
                            ...
                            {confirmNewPassword && newPassword === confirmNewPassword && (
                                <p style={{ color: '#34D399', fontSize: '12px', marginTop: '5px', fontWeight: '600' }}>Passwords match</p>
                            )}
                        </div>
