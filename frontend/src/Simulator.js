...
const DISRUPTIONS = [
    { label: 'Heavy Rain — 78mm', type: 'rain', severity: 'Moderate', value: '78mm rainfall', pct: 0.65, icon: '🌧️', color: '#1A56A0' },
...
function Simulator({ worker, onBack }) {
    ...
    return (
        <div style={{ backgroundColor: '#f5f7fa', minHeight: '100vh', fontFamily: 'Arial' }}>
            ...
            {/* Navbar */}
            <div style={{ backgroundColor: '#1A56A0', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ padding: '20px', maxWidth: '580px', margin: '0 auto' }}>
                ...
                <h2 style={{ color: '#1A56A0', fontSize: '22px', fontWeight: 'bold', marginBottom: '4px' }}>⚡ Live Disruption Simulator</h2>
                        ...
                        {steps.map((s, i) => (
                            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                                <div style={{
                                    ...
                                    backgroundColor: currentStep > i ? '#1A56A0' : currentStep === i ? '#F0A500' : '#f0f0f0',
                                ...
                                <p style={{ fontSize: '10px', color: currentStep >= i ? '#1A56A0' : '#aaa', marginTop: '4px', textAlign: 'center' }}>{s.label}</p>
                                {i < steps.length - 1 && (
                ...
                {phase === 'setup' && (
                    <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', marginBottom: '16px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                                {Object.keys(ZONE_WORKERS).map(z => (
                                    <div key={z} onClick={() => setSelectedZone(z)}
                                        style={{ padding: '12px', borderRadius: '10px', border: `2px solid ${selectedZone === z ? '#1A56A0' : '#ddd'}`, backgroundColor: selectedZone === z ? '#D6E4F7' : 'white', cursor: 'pointer', textAlign: 'center' }}>
                                        <p style={{ color: selectedZone === z ? '#1A56A0' : '#555', fontWeight: 'bold', fontSize: '12px' }}>{z.split(',')[0]}</p>
                                        <p style={{ color: '#888', fontSize: '11px', marginTop: '2px' }}>{z.split(',')[1]?.trim()}</p>
                                        <p style={{ color: '#1A56A0', fontSize: '11px', marginTop: '4px' }}>{ZONE_WORKERS[z].length} workers</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div style={{ backgroundColor: '#D6E4F7', borderRadius: '12px', padding: '16px', marginBottom: '20px' }}>
                            <p style={{ color: '#1A56A0', fontWeight: 'bold', fontSize: '14px', marginBottom: '8px' }}>📊 Simulation Preview</p>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                                {[
                                    ...
                                    <div key={i} style={{ backgroundColor: 'white', borderRadius: '8px', padding: '10px', textAlign: 'center' }}>
                                        <p style={{ color: '#1A56A0', fontWeight: 'bold', fontSize: '16px' }}>{item.value}</p>
                        ...
                        <button onClick={runSimulation}
                            style={{ width: '100%', backgroundColor: '#1A56A0', color: 'white', padding: '16px', borderRadius: '12px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}>
                            ▶ Run Simulation
                        </button>
                    </div>
                )}
                    ...
                    <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '20px', marginBottom: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
                        ...
                        {phase === 'running' && workerCards.length < workers.length && (
                            <div style={{ textAlign: 'center', padding: '16px' }}>
                                <p style={{ color: '#1A56A0', fontSize: '13px' }}>🔄 Verifying remaining workers...</p>
                            </div>
                        )}
                    </div>
                )}
                ...
                {/* Summary Card */}
                {summary && (
                    <div style={{ backgroundColor: '#1A56A0', borderRadius: '16px', padding: '24px', marginBottom: '16px', color: 'white', boxShadow: '0 4px 20px rgba(26,86,160,0.3)' }}>
                ...
                {phase === 'done' && (
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
                        <button
                            ...
                            style={{ flex: 1, backgroundColor: '#1A56A0', color: 'white', padding: '14px', borderRadius: '10px', border: 'none', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer' }}>
                            🔄 Run Again
                        </button>
                        <button onClick={onBack}
                            style={{ flex: 1, backgroundColor: 'white', color: '#1A56A0', padding: '14px', borderRadius: '10px', border: '2px solid #1A56A0', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer' }}>
                            ← Dashboard
                        </button>
                    </div>
                )}
